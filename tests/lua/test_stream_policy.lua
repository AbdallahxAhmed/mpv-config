...
test('analyze cache coverage accurately identifies complete and partial ranges', function()
    local cov1 = p.analyze_cache_coverage({bof = true, eof = true, ['seekable-ranges'] = {{start = 0, ['end'] = 100}}}, 100)
    eq(cov1.is_complete, true)
    eq(cov1.coverage_pct, 100)
    eq(cov1.start_time, 0)
    eq(cov1.end_time, 100)

    local cov2 = p.analyze_cache_coverage({bof = true, eof = true, ['seekable-ranges'] = {{start = 1, ['end'] = 99}}}, 100)
    eq(cov2.is_complete, true)
    eq(cov2.coverage_pct, 98)

    local cov3 = p.analyze_cache_coverage({bof = true, eof = true, ['seekable-ranges'] = {{start = 0, ['end'] = 30}}}, 100)
    eq(cov3.is_complete, false)
    eq(cov3.coverage_pct, 30)

    local cov4 = p.analyze_cache_coverage({bof = false, eof = true, ['seekable-ranges'] = {{start = 10, ['end'] = 100}}}, 100)
    eq(cov4.is_complete, false)

    local cov5 = p.analyze_cache_coverage({bof = true, eof = false, ['seekable-ranges'] = {{start = 0, ['end'] = 95}}}, 100)
    eq(cov5.is_complete, false)
    eq(cov5.coverage_pct, 95)
